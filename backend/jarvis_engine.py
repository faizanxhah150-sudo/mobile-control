...
# --- CORS ---
# The Jarvis app's WebView runs the frontend under its own origin
# (capacitor://localhost / https://localhost), so every request to this
# local Flask server is cross-origin as far as the browser is concerned.
# Without these headers the browser's CORS check silently blocks the
# real response after the OPTIONS preflight succeeds, and fetch() just
# throws "Failed to fetch" with no useful detail on the app side.
@app.after_request
    ...
    try:
        ...
        if action_result is not None:
            if action_result.get("ok"):
                confirmation = action_result.get("message") or "Ho gaya."
            else:
                confirmation = action_result.get("error") or "Yeh nahi ho saka."
            synthesize_and_play(confirmation)
            return jsonify({"ok": True, "type": "action", "action_result": action_result, "reply": confirmation})
